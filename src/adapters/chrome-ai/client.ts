...
export class ChromeAIClient implements ChromeAIAdapter {
  ...
  async checkCapabilities(): Promise<AICapabilities> {
    ...
    try {
      ...
      return {
        ...
        chiefComplaint: parsed.chiefComplaint || 'Patient presenting for evaluation',
  ...
  private buildDialoguePrompt(message: string, context: DialogueContext): string {
    ...
    return `You are a patient in a medical consultation.${scenarioContext}
...
Respond naturally as the patient. Format as JSON:
{
  "message": "Your response",
  "emotion": "calm|anxious|relieved|confused",
  "suggestions": ["possible question 1", "possible question 2"]
}`;
  }
  ...
  private parseGrammarResult(original: string, corrected: string): GrammarCheckResult {
  ...
  private async checkTranslatorAPI(): Promise<'available' | 'downloading' | 'unavailable'> {
  ...
  private async checkRewriterAPI(): Promise<'available' | 'downloading' | 'unavailable'> {
      ...
      try {
        if (!this.languageModel) {
          this.languageModel = await window.ai!.languageModel!.create({
            systemPrompt: 'You are a medical education assistant. Generate realistic clinical scenarios for medical students.',
            temperature: 0.8,
          });
        }
        ...
        logger.error('ai.scenario.error', { params, error, duration: Date.now() - start });
  ...
  async simulateDialogue(
    ...
    return this.withRetry(async () => {
      try {
        if (!this.languageModel) {
          this.languageModel = await window.ai!.languageModel!.create({
            systemPrompt: 'You are a patient in a medical scenario. Respond realistically to the doctor\'s questions.',
            temperature: 0.9,
          });
        }
        ...
        logger.error('ai.dialogue.error', { error, duration: Date.now() - start });
  ...
  async translateTerm(params: TranslationParams): Promise<AIResult<TranslationResult>> {
    const start = Date.now();
